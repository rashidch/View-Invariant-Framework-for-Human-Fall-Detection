...
def train(stmodel,aemodel,dataloader, dataset_size, num_channel, num_point, n_frames, num_epochs, fold=None):
    ...
    optimizer = optimizer_choose2(stmodel,aemodel, optimizer='sgd_nev')
        ...
        for phase in ["train", "valid"]:
            ...
            for _, (inputs, targets) in enumerate(dataloader[phase]):
                ...
                with torch.set_grad_enabled(phase == "train"):
                    Deinputs = aemodel(inputs)
                    stDeinputs = Deinputs.reshape(-1,num_frame,num_point,num_channel).permute(0,3,1,2).contiguous()
                    stinputs = inputs.reshape(-1,num_frame,num_point,num_channel).permute(0,3,1,2).contiguous()
                    # compute model outputs
                    De_preds, De_probs  = stmodel(stDeinputs)
                    raw_preds, class_probs = stmodel(stinputs)
                    # calculate outputs
                    _, Depreds = torch.max(De_probs, dim=1)
                    _, preds = torch.max(class_probs, dim=1)
                    # calculate the loss
                    loss_Depreds = criterion(De_preds,targets)
                    loss_Outpreds = criterion(raw_preds, targets)
                    loss_mse = MSEdis(Deinputs, inputs)

                    loss  = loss_Depreds + loss_Outpreds + loss_mse*lamda
                ...
                running_corrects += torch.sum(Depreds == targets)

                # Append batch prediction results
                Depred_tensor= torch.cat([Depred_tensor, Depreds.view(-1).cpu()])
            ...
            precision1 = precision_score(class_tensor.numpy(), Depred_tensor.numpy(), average=None)
            recall1 = recall_score(class_tensor.numpy(), Depred_tensor.numpy(), average=None)
            f_score1 = f1_score(class_tensor.numpy(), Depred_tensor.numpy(), average=None)
            ...
            print('{} : De Precision per class: {}'.format(phase, np.round(precision1,4)))
            print('{} : De Recall per class: {}'.format(phase, np.round(recall1,4)))
            print("{} : De F1_Score per class: {}".format(phase, np.round(f_score1, 4)))
            print()
            ...
            if phase == "valid" and loss_ > epoch_loss:
                ...
                saveModel(
                    aemodel, optimizer, loss_, epoch_acc, epoch_,fold,save_path=r"checkpoints/AEnet_"+ str(currTime)
                )
    ...
    saveModel(
    aemodel, optimizer, epoch_loss, epoch_acc, epoch,fold,save_path=r"checkpoints/AEnet_"+str(currTime))
...
def trainvalSplit(stnet,AEGen,num_channel, num_point,n_frames,bs, num_epochs):
    ...
    # train the model
    history, _, conf_train, conf_valid = train(stnet,AEGen, dataLoader, datasetSize, num_channel,
...
if __name__ == "__main__":
   ...
   '''
    ...
    AEGen = GenNet(Num).to(device)
    
    if training_mode == "train_test_split":
        trainvalSplit(stnet,AEGen,num_channel=num_channel, num_point=num_point,n_frames=num_frame, bs=batchSize, num_epochs=num_epochs)
