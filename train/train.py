...
if __name__ == '__main__':
    ...
    #LSTM_model   = FallModel(input_dim=24, class_num=3).to(device)
    DNN_model   = FallModel(input_dim=24, class_num=3).to(device)
    total_params = sum(p.numel() for p in DNN_model.parameters() if p.requires_grad)
    print(total_params)

    #get test dataloaders
    dataloaders, dataset_sizes = prepare_data(bs=32, seq_len=5)
    history, model, conf_train, conf_valid = train_model(DNN_model, dataloaders, dataset_sizes,num_epochs=1000)
