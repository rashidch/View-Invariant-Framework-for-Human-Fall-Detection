...
class DetectionLoader():
    def __init__(self, input_source, detector, cfg, opt, mode='image', batchSize=1, queueSize=128):
        ...
        #save directory path to the class name from folder name
    ...
    def image_preprocess(self):
        for i in range(self.num_batches):
            ...
            self.wait_and_put(self.image_queue, (imgs, orig_imgs, im_names, im_dim_list, im_url_k))
    ...
    def frame_preprocess(self):
        ...
        for i in range(self.num_batches):
            ...
            self.wait_and_put(self.image_queue, (imgs, orig_imgs, im_names, im_dim_list, None))
        stream.release()

    def image_detection(self):
        for i in range(self.num_batches):
            imgs, orig_imgs, im_names, im_dim_list, im_url_k = self.wait_and_get(self.image_queue)
            ...
            for k in range(len(orig_imgs)):
                ...
                if im_url_k==None:
                    self.wait_and_put(self.det_queue, (orig_imgs[k], im_names[k],boxes_k, scores[dets[:, 0] == k], ids[dets[:, 0] == k], inps, cropped_boxes, None))
                else:
                    self.wait_and_put(self.det_queue, (orig_imgs[k], im_names[k],boxes_k, scores[dets[:, 0] == k], ids[dets[:, 0] == k], inps, cropped_boxes, im_url_k[k]))
    def image_postprocess(self):
        for i in range(self.datalen):
            with torch.no_grad():
                (orig_img, im_name, boxes, scores, ids, inps, cropped_boxes, im_url) = self.wait_and_get(self.det_queue)
                ...
                self.wait_and_put(self.pose_queue, (inps, orig_img, im_name, boxes, scores, ids, cropped_boxes,im_url))
