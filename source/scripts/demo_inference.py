...
parser.add_argument('--qsize', type=int, dest='qsize', default=520,
...
if __name__ == "__main__":
    ...
    try:
        for i in im_names_desc:
            start_time = getTime()
            with torch.no_grad():
                (inps, orig_img, im_name, boxes, scores, ids, cropped_boxes, im_url) = det_loader.read()
                if orig_img is None:
                    break
                ...
                hm = hm.cpu()
                #print('Class IDs', ids, 'Image Name:', im_name)
                writer.save(boxes, scores, ids, hm, cropped_boxes, orig_img, im_name, im_url)
                if args.profile:
